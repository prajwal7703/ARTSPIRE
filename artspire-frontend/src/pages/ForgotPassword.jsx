...
function NotifBar({ notif, onClose }) {
  ...
  return (
    <div style={{
      ...
      {notif.type !== "loading" && (
        <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", fontSize:"18px", color:theme.color, opacity:0.6, padding:0, lineHeight:1 }}>x</button>
      )}
    </div>
  );
}
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async () => {
    if (!email.trim()) {
      showNotif("error", "Please enter your email address.");
      return;
    }
