...
// ── Safely extract MongoDB _id whether it's a string or { $oid: "..." } ──────
function getId(obj) {
...
function getSkills(cat) {
  return CATEGORY_SKILLS[cat] || CATEGORY_SKILLS.default;
}
...
export default function ArtistProfile() {
  ...
  try { loggedArtist = loggedArtistRaw ? JSON.parse(loggedArtistRaw) : null; } catch { loggedArtist = null; }

  let loggedUser = null;
  try { loggedUser = loggedUserRaw ? JSON.parse(loggedUserRaw) : null; } catch { loggedUser = null; }

  const isLoggedIn = !!(loggedArtist || loggedUser);
  const role = loggedArtist ? "artist" : loggedUser ? "user" : null;

  // ── isOwner uses getId() so { $oid } objects compare correctly ──
  ...
  const fetchArtist = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/artists/${id}`);
      setArtist(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  if (!artist) {
    return (
      <div style={{ background: "#f0f4ff", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "36px", color: "#1e3a8a", letterSpacing: "3px" }}>
          LOADING PROFILE...
        </div>
      </div>
    );
  ...
  return (
    <div style={s.page}>
      ...
      <style>{`
        ...
        .post-thumb { position: relative; cursor: pointer; }
        .post-thumb:hover { transform: scale(1.04); box-shadow: 0 8px 30px rgba(30,58,138,0.25); }
        .post-thumb .thumb-overlay { pointer-events: none; }
        ...
        @media (max-width: 600px) {
          ...
          .blue-section-content { padding: 32px 20px !important; }
          .action-row-wrap { flex-direction: column !important; }
          .action-row-wrap button { width: 100%; justify-content: center; }
        }
      `}</style>
      ...
      <div style={s.heroSection}>
        ...
        <div style={s.topNav}>
          ...
          <div style={s.navLinks}>
            ...
            <span style={s.navItem}>Portfolio</span>
          </div>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button style={{ ...s.navActionBtn, background: "#dc2626" }} onClick={handleLogout}>
              Logout
            </button>
          </div>
        </div>
        ...
        <div style={s.heroContent}>
          ...
          <div style={s.heroLeft}>
            ...
            <div style={s.aboutSection}>
              ...
              <p style={s.bioText}>
                {artist.bio || `${artist.name} is a talented ${artist.category || "artist"} based in ${artist.city || "the world"}. Their work reflects a deep passion for their craft and a commitment to creative excellence.`}
              </p>
          ...
          <div style={s.heroRight}>
            ...
            <div style={s.statsCard}>
              <div style={s.statItem}>
                ...
                <div style={s.statLbl}>Posts</div>
              </div>
              <div style={s.statDivider} />
              <div style={s.statItem}>
                <div style={s.statNum}>{artist.city ? artist.city.slice(0, 3).toUpperCase() : "—"}</div>
                <div style={s.statLbl}>City</div>
              </div>
              <div style={s.statDivider} />
              <div style={s.statItem}>
                <div style={s.statNum}>'25</div>
                <div style={s.statLbl}>Year</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="blue-section-content" style={s.blueSectionContent}>
          ...
          <div style={{ marginBottom: "36px", animation: "fadeUp 0.5s ease both", animationDelay: "0.1s" }}>
            ...
            <div style={s.skillsGrid}>
              {skills.map((skill, i) => (
                <span key={i} className="skill-pill" style={s.skillPill}>{skill}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{ animation: "fadeUp 0.5s ease both", animationDelay: "0.2s" }}>
            ...
            {posts.length === 0 ? (
              <div style={s.emptyPortfolio}>
                ...
                <div style={{ color: "rgba(255,255,255,0.6)", fontFamily: "'Nunito', sans-serif", fontSize: "15px" }}>
          ...
          <div className="action-row-wrap" style={s.actionRow}>
            ...
            {role === "user" && (
              <>
                <button className="action-btn" style={s.primaryBtn} onClick={() => navigate(`/chat/${id}`)}>
                  💬 Chat with Artist
                </button>
                <button className="action-btn" style={s.secondaryBtn} onClick={() => navigate("/artists")}>
                  🔍 More Artists
                </button>
              </>
            )}
            ...
            {isOwner && (
              <>
                <button className="action-btn" style={s.primaryBtn} onClick={() => navigate("/artist-dashboard")}>
                  🛠 Dashboard
                </button>
                <button className="action-btn" style={s.secondaryBtn} onClick={() => navigate(`/chat/${getId(artist)}`)}>
                  💬 Chat
                </button>
              </>
            )}

            <button className="action-btn" style={s.secondaryBtn} onClick={() => navigate("/")}>
              🏠 Home
            </button>

            <button
              className="action-btn"
              style={{ ...s.secondaryBtn, background: "#dc2626", borderColor: "#dc2626" }}
...
const s = {
  ...
  statsCard: {
    ...
    padding: "12px 20px",
    marginTop: "16px",
    boxShadow: "4px 4px 0 rgba(30,58,138,0.3)",
  },
  ...
  // ── BLUE SECTION ──
  blueSection: {
    background: "linear-gradient(160deg, #1e40af 0%, #1e3a8a 40%, #1d4ed8 100%)",
    display: "flex",
    position: "relative",
    minHeight: "600px",
  },
