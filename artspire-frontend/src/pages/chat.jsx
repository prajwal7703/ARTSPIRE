...
const Toast = ({ notif, onClose }) => {
  ...
  return (
    <div style={{
      ...
      <button onClick={onClose} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: "16px", padding: "2px" }}>?</button>
    </div>
  );
};
...
export default function Chat() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!raw || raw === "undefined" || raw === "null") {
        parsedUser = { _id: id, name: "Me", role: "guest" };
      } else {
      ...
      try {
        ...
        let others = parsedUser.role === "artist"
          ? allPeople.filter(p => p.role === "user")
          : allPeople.filter(p => p.role === "artist");

        const mapped = others.map(p => ({
          id: p._id, name: p.name, role: p.role,
          status: "Online", image: p.profileImage || null,
          category: p.category || "Artist",
        }));
        setUsers(mapped);

        const urlArtist = allPeople.find(p => p._id === id && p._id !== parsedUser._id);
        if (urlArtist) {
          setSelectedUser({ id: urlArtist._id, name: urlArtist.name, role: urlArtist.role, status: "Online", image: urlArtist.profileImage || null, category: urlArtist.category || "Artist" });
          setReceiverId(urlArtist._id);
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/chat/${currentUser._id}/${receiverId}`
      );
    ...
    const handler = (data) => {
      ...
      if (isCurrentChat) {
        ...
        if (Notification.permission === "granted") {
          new Notification("New message", { body: data.message || "?? Media", icon: "/favicon.ico" });
        }
      }
    };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (imagePreview?.file) {
        ...
        finalMsg.mediaUrl = uploadRes.data.url || uploadRes.data.imageUrl || uploadRes.data.path;
      }

      await axios.post(`${import.meta.env.VITE_API_URL}/api/chat/send`, finalMsg);
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        const tempId = Date.now().toString();
        const audioMsg = {
          _id: tempId, senderId: currentUser._id, receiverId,
  ...
  const selectUser = (user) => { setSelectedUser(user); setReceiverId(user.id); setMessages([]); setUnreadMap(prev => ({ ...prev, [user.id]: 0 })); };
      ...
      <div style={s.sidebar}>
        <div style={s.sidebarHeader}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button className="icon-action" style={s.iconAction} onClick={() => navigate(-1)}>
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><line x1="3" y1="12" x2="21" y2="12"/><polyline points="3,6 3,18"/></svg>
            </button>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <div style={s.sidebarTitle}>Messages</div>
            {totalUnread > 0 && (
              <div style={{ background: "#ef4444", borderRadius: "10px", padding: "2px 7px", fontSize: "11px", fontWeight: 700, color: "#fff" }}>{totalUnread}</div>
            )}
          </div>
        ...
        <div style={s.filterBar}>
          <div style={s.dateFilter}>Today <span style={{ opacity: 0.5, marginLeft: "4px" }}>?</span></div>
          <div style={s.dateFilter}>Date <span style={{ opacity: 0.5, marginLeft: "4px" }}>?</span></div>
        </div>

        <div style={s.searchWrap}>
          <svg style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", opacity: 0.4 }} width="14" height="14" fill="none" stroke="#fff" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search conversations..." style={s.searchInput} />
        </div>

        <div style={s.usersList}>
          {filteredUsers.length === 0 ? (
            <div style={{ padding: "20px", color: "#475569", fontSize: "13px", textAlign: "center" }}>No conversations yet</div>
          ) : (
            filteredUsers.map(user => (
              <div
                ...
                style={{
                  ...s.userItem,
                  background: selectedUser?.id === user.id ? "rgba(99,102,241,0.15)" : "transparent",
                  borderLeft: selectedUser?.id === user.id ? "3px solid #6366f1" : "3px solid transparent",
                }}
              >
                ...
                <div style={s.userInfo}>
                  ...
                  <div style={s.userSubRow}>
                    ...
                    {(unreadMap[user.id] || 0) === 0 && (
                      <span style={{ fontSize: "11px", color: "#38bdf8", letterSpacing: "-2px" }}>??</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div style={s.chatArea}>
          <div style={s.chatHeader}>
            <div style={s.chatHeaderLeft}>
              ...
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#22c55e", fontFamily: "'Outfit', sans-serif" }}>? Online</div>
              </div>
            </div>
            <div style={s.chatHeaderActions}>
              <button className="icon-action" style={s.headerIconBtn} onClick={() => navigate(`/artist-profile/${selectedUser.id}`)}>
              ...
              <button className="icon-action" style={s.headerIconBtn}>
                <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
              </button>
              <button className="icon-action" style={s.headerIconBtn}>
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <div key={i} style={{ ...s.msgRow, justifyContent: isMine ? "flex-end" : "flex-start", animation: "msgIn 0.2s ease both" }}>
                  {!isMine && <Avatar image={selectedUser.image} name={selectedUser.name} size={28} />}
          ...
          {imagePreview && (
            <div style={s.previewBar}>
              ...
              <button onClick={() => setImagePreview(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#ef4444" }}>?</button>
            </div>
          )}

          <div style={s.inputArea}>
            <div style={s.inputLeft}>
              <button className="icon-action" style={s.inputIconBtn} title="Upload Image" onClick={() => fileInputRef.current.click()}>
            ...
            <div style={s.inputRight}>
              ...
              <button
                style={{ ...s.sendBtn, opacity: (text.trim() || imagePreview) && !sending ? 1 : 0.4 }}
                onClick={sendMessage}
...
const s = {
  ...
  iconAction: { background: "rgba(255,255,255,0.06)", border: "none", width: "34px", height: "34px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#94a3b8", transition: "background 0.2s" },
  ...
  messagesArea: { flex: 1, overflowY: "auto", padding: "24px 20px 12px", display: "flex", flexDirection: "column", gap: "6px", background: "#f8fafc" },
  ...
  textInput: { flex: 1, background: "none", border: "none", outline: "none", fontSize: "14px", color: "#1e293b", fontFamily: "'Outfit', sans-serif", resize: "none", padding: "8px 4px", maxHeight: "100px", lineHeight: 1.5 },
