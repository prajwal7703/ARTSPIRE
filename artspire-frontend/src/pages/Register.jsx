...
// ── Notification bar (matches ArtistRegister) ─────────────────────────────────
const NOTIF_THEMES = {
...
// ── Step dots (matches ArtistRegister) ────────────────────────────────────────
    ...
    <div style={{ display:"flex", gap:"8px", justifyContent:"center", marginBottom:"28px" }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{
          ...
          height: "8px",
          borderRadius: "4px",
          background: i === step ? "#E8192C" : i < step ? "#86efac" : "#d1d5db",
...
export default function Register() {
  ...
  const [step, setStep] = useState(0); // 0 = account, 1 = preferences
                ...
                <div style={s.fields}>
                        ...
                        <div style={{ display: "flex", gap: "4px" }}>
                          {[1, 2, 3].map((lvl) => (
                            <div key={lvl} style={{
                              ...
                              background: passwordStrength >= lvl ? strengthColors[passwordStrength - 1] : "#e2e8f0",
                              transition: "background 0.3s",
                            }} />
                          ))}
                        </div>
                        <div style={{ fontSize: "11px", color: strengthColors[passwordStrength - 1], fontWeight: 700, marginTop: "4px" }}>
                  ...
                  <div style={s.fieldGroup}>
                    ...
                    {form.confirmPassword && form.password !== form.confirmPassword && (
                      <div style={{ fontSize: "12px", color: "#ef4444", fontWeight: 700, marginTop: "4px" }}>Passwords do not match</div>
                    )}
                    {form.confirmPassword && form.password === form.confirmPassword && form.password.length >= 6 && (
                      <div style={{ fontSize: "12px", color: "#22c55e", fontWeight: 700, marginTop: "4px" }}>✓ Passwords match</div>
                    )}
                  </div>
                </div>
            ...
            {step === 1 && (
              <div style={s.stepWrap}>
                    ...
                    <div style={s.categoryGrid}>
                      {INTERESTS.map((item) => (
                        <button
                          ...
                          style={{ ...s.catBtn, ...(form.interests.includes(item) ? s.catBtnActive : {}) }}
                        >
                          {item}
                        </button>
                      ))}
                ...
                <div style={{ display:"flex", gap:"12px" }}>
                  ...
                  <button style={{ ...s.primaryBtn, flex:1 }} onClick={handleSubmit}>
                    🎉 Join ArtSpire
                  </button>
                </div>
              </div>
...
// ── Styles (mirrors ArtistRegister, accent swapped to #E8192C) ─────────────────
const s = {
  ...
  googleBtn: { width:"100%", padding:"13px 24px", border:"1.5px solid #e2e8f0", borderRadius:"50px", background:"#fff", color:"#1e293b", fontSize:"14px", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Nunito', sans-serif", marginTop:"0" },
