...
export default function ArtistLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ backend returns both user and artist keys for artist accounts
      const data   = res.data.artist || res.data.user;
      const token  = res.data.token;

      if (!data) { showNotif("error", "Login failed. Please try again."); return; }

      // ✅ Make sure it's an artist account
      if (data.role !== "artist") {
        showNotif("error", "This is not an artist account. Please use User Login.");
        return;
      }
      ...
      showNotif("artist", `Welcome back, ${data.name}! 🎨`);
      ...
      <div style={s.page}>
        ...
        <div style={s.left}>
          ...
          <div style={s.leftContent}>
            ...
            <div style={s.perks}>
              {[{ icon:"🎨",text:"Showcase your portfolio"},{icon:"🌍",text:"Connect with local fans"},{icon:"📅",text:"Manage your bookings"},{icon:"💬",text:"Direct messages from users"}].map(p=>(
        ...
        <div style={s.right}>
          <div style={s.formCard}>
            <div style={s.topLink}>Not an artist?{" "}<span style={s.topLinkBtn} onClick={() => navigate("/login")}>User Login</span></div>
            ...
            <form onSubmit={handleSubmit} style={s.form}>
              <div style={s.fieldGroup}>
                ...
                <input type="email" name="email" placeholder="you@example.com" value={formData.email} onChange={handleChange} required autoComplete="username" style={s.input} />
              </div>
              <div style={s.fieldGroup}>
                <label style={s.fieldLabel}>Password</label>
                <input type="password" name="password" placeholder="Your password" value={formData.password} onChange={handleChange} required autoComplete="current-password" style={s.input} />
              </div>
              <p onClick={() => navigate("/forgot-password")} style={{ textAlign:"right",color:"#4f46e5",cursor:"pointer",fontWeight:700,fontSize:"13px",margin:"-4px 0 4px" }}>Forgot password?</p>
              <button type="submit" style={s.primaryBtn}>🎨 Sign In as Artist</button>
            </form>
            <div style={{ textAlign:"center",marginTop:"20px",fontSize:"14px",color:"#94a3b8",fontWeight:600 }}>
              New artist?{" "}<span onClick={() => navigate("/artist-register")} style={{ color:"#4f46e5",cursor:"pointer",fontWeight:800 }}>Create Artist Profile →</span>
            </div>
          </div>
        </div>
      </div>
