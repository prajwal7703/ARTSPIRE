...
export default function ArtistLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.user?.role !== "artist") {
        showNotif("error", "This account is not an artist account. Please use the user login.");
        return;
      }
  ...
  // ── Google login — POPUP (avoids unauthorized-domain on Vercel previews) ──
  const googleLogin = async () => {
    ...
    try {
      ...
      // Register/login as artist via Google
      const res = await axios.post(`${API}/api/auth/google`, {
        name:  user.displayName,
        email: user.email,
        photo: user.photoURL,
        role:  "artist", // marks them as artist on first Google signup
      });
      ...
      console.error("Google artist login error:", err);
    }
  };
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1e3a8a,#1d4ed8,#312e81)", display:"flex", justifyContent:"center", alignItems:"center", padding:"30px" }}>
        <div style={{ width:"100%", maxWidth:"1100px", background:"white", borderRadius:"40px", overflow:"hidden", display:"grid", gridTemplateColumns:"1fr 1fr", boxShadow:"0 20px 60px rgba(0,0,0,0.4)" }}>
          ...
          {/* LEFT */}
          <div style={{ background:"#f0f4ff", padding:"40px", position:"relative", display:"flex", justifyContent:"center", alignItems:"center" }}>
            <div style={{ position:"absolute", top:"20px", left:"30px", display:"flex", alignItems:"center", gap:"10px" }}>
              ...
              <h1 style={{ fontSize:"38px", fontWeight:"bold", color:"#1e3a8a" }}>ArtSpire</h1>
            </div>
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:"80px", marginBottom:"16px" }}>🎨</div>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"28px", color:"#1e3a8a", letterSpacing:"2px", marginBottom:"8px" }}>Artist Portal</div>
          ...
          <div style={{ padding:"60px", display:"flex", justifyContent:"center", alignItems:"center" }}>
            <form onSubmit={handleSubmit} style={{ width:"100%", maxWidth:"400px", display:"flex", flexDirection:"column", gap:"20px" }}>
              ...
              <h1 style={{ textAlign:"center", fontSize:"34px", color:"#1e3a8a", fontWeight:"bold", margin:0 }}>Artist Login</h1>
...
const primaryBtn = { padding:"16px", border:"none", borderRadius:"50px", background:"linear-gradient(90deg,#1e3a8a,#1d4ed8)", color:"white", fontSize:"18px", fontWeight:"bold", cursor:"pointer" };
