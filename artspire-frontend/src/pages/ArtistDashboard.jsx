...
function Toast({ msg, type, onClose }) {
  ...
  if (!msg) return null;
  const map = {
    success: "#dcfce7|#14532d|#86efac",
    error: "#fee2e2|#7f1d1d|#fca5a5",
    info: "#dbeafe|#1e3a5f|#93c5fd",
  };
...
export default function ArtistDashboard() {
  ...
  // ✅ FIX: Get the correct ID field — handles both _id and id
  const getArtistId = () => artist?._id || artist?.id;

  const handleViewProfile = () => {
    const id = getArtistId();
    if (!id) {
      showToast("Profile ID not found. Please log in again.", "error");
      return;
    }
        ...
        <div style={s.headerInner}>
          <div style={s.profileRow}>
            ...
            <div style={s.headerActions}>
              {/* ✅ FIXED: View Profile button now uses handleViewProfile with fallback */}
              <button
