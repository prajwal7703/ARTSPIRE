...
export default function BookingModal({ artist, currentUser, onClose, onSuccess }) {
  ...
  // ── Listen for artist's offer via socket ──────────────────────────────────
  useEffect(() => {
    if (!bookingId) return;
    ...
    socket.on("price_accepted",  handlePriceAccepted);
    socket.on("booking_cancelled", handleCancelled);

    return () => {
      socket.off("booking_offer",    handleOffer);
      socket.off("price_accepted",   handlePriceAccepted);
  ...
  const submitRequest = async () => {
    if (!form.eventType || !form.eventDate || !form.location) {
      setError("Please fill Event Type, Date, and Location.");
      return;
    }
    ...
    try {
      ...
      // ✅ FIX: backend returns _id not bookingId
  ...
  const sendCounter = async () => {
    ...
    try {
      ...
      setThread(t => [...t, {
        from: "user", price: amt, message: counterMsg, ts: new Date(),
      }]);
  ...
  const acceptPrice = async (price) => {
    ...
    try {
      // ✅ FIX: correct route is /user-accept not /accept
      await axios.post(`${API}/api/bookings/${bookingId}/user-accept`, { price });
  ...
  const handlePay = async () => {
    ...
    setError(""); setLoading(true);
    try {
      // ✅ FIX: send bookingId, not amount — backend reads agreedPrice from DB
      const orderRes = await axios.post(`${API}/api/bookings/create-order`, {
        bookingId,
      });
      const { orderId, amount: orderAmount, keyId, demo } = orderRes.data;
  ...
  const confirmPayment = async (paymentId, orderId, amount, signature) => {
    try {
      // ✅ FIX: correct body fields matching backend
      ...
      setError(
        ...
        "Booking confirmation failed. Contact support if money was deducted."
      );
    } finally { setLoading(false); }
  };
      ...
      <div style={{ ...s.modal, textAlign: "center" }}>
        ...
        <div style={{ padding: "40px 28px 28px" }}>
          ...
          <div style={{ fontSize: 14, color: "#64748b", fontFamily: "'Nunito',sans-serif", marginBottom: 20 }}>
            ...
            <strong>{artist.name}</strong> is confirmed for your{" "}
          ...
          <div style={s.card}>
            {[
              ...
              payMode === "advance" ? ["Balance on Event Day", `₹${balanceAmount?.toLocaleString()}`] : null,
        ...
        <div style={s.stepper}>
          {[["Event", 1], ["Price", 2], ["Payment", 3]].map(([lbl, n], i) => (
            <div key={n} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {i < 2 && (
                <div style={{ width: 28, height: 2, background: step > n ? "#16a34a" : "#e2e8f0", borderRadius: 2, marginLeft: 2 }} />
              )}
            </div>
          ))}
        </div>
          ...
          <div style={s.body}>
                ...
                {basePrice > 0
                  ? <>
                      ...
                      <div style={{ fontSize: 12, color: "#3b82f6", fontFamily: "'Nunito',sans-serif", marginTop: 2 }}>
                        Artist will confirm or adjust this for your event. You can negotiate.
                      </div>
                    </>
                  : <>
                      <div style={{ fontWeight: 800, fontSize: 13, color: "#1d4ed8", fontFamily: "'Nunito',sans-serif" }}>
                      ...
                      <div style={{ fontSize: 12, color: "#3b82f6", fontFamily: "'Nunito',sans-serif", marginTop: 2 }}>
                        Artist will quote a price after reviewing your event details.
                      </div>
                    </>
                }
              </div>
            ...
            <div style={s.field}>
              ...
              <textarea
                ...
                placeholder="Tell the artist about your event — audience size, requirements, vibe…"
                rows={3}
                className="bmi"
          ...
          <div style={s.body}>
              ...
              <div ref={threadRef} style={s.thread}>
                  ...
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.from === "user" ? "flex-end" : "flex-start", gap: 2 }}>
                    ...
                    <div style={{
                      ...
                      {msg.price > 0 && (
                        <span style={{
                          display: "inline-flex",
                          background:   msg.from === "user" ? "rgba(255,255,255,0.18)" : "#f0fdf4",
                          color:        msg.from === "user" ? "#fff" : "#15803d",
                ...
                {negoStatus === "waiting" && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, paddingTop: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#94a3b8", fontFamily: "'Nunito',sans-serif" }}>
                      {artist.name} is reviewing your request…
                    </span>
                  </div>
                )}
              </div>
            ...
            {negoStatus === "agreed" && agreedPrice && (
              <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 12, padding: "12px 14px", display: "flex", gap: 10, alignItems: "center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#16a34a", fontFamily: "'Nunito',sans-serif" }}>
                    Proceed to choose your payment method
                  </div>
                </div>
              </div>
            )}

            {/* Accept artist's price */}
            {negoStatus === "replied" && lastArtistMsg && (
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button
                  onClick={() => acceptPrice(lastArtistMsg.price)}
                  disabled={loading}
                  style={{ flex: 1, background: "#16a34a", color: "#fff", border: "none", padding: "11px 14px", borderRadius: 18, fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13, cursor: "pointer", opacity: loading ? 0.7 : 1 }}
                >
                  {loading ? "…" : `✓ Accept ₹${lastArtistMsg.price?.toLocaleString()}`}
                </button>
              </div>
            )}
            ...
            {negoStatus === "replied" && (
              <div style={{ background: "#faf5ff", border: "1px solid #e9d5ff", borderRadius: 12, padding: "12px 14px" }}>
                <div style={s.sectionLbl}>Or send a counter offer</div>
                <div className="bmg" style={{ ...s.grid, marginBottom: 8 }}>
                  <div style={s.field}>
                    ...
                    <input
                      ...
                      className="cni"
                      style={{ ...s.input, borderColor: "#c4b5fd" }}
                    />
                  </div>
                  <div style={s.field}>
                    ...
                    <input
                      ...
                      placeholder="Why this price works…"
                      className="cni"
                      style={{ ...s.input, borderColor: "#c4b5fd" }}
                    />
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {[0.85, 0.9, 0.95].map(pct => {
                      ...
                      return (
                        <button
                          key={pct}
                          onClick={() => { setCounterAmt(p); setCounterMsg(`Would ₹${p.toLocaleString()} work?`); }}
                          style={s.chip}
                        >
                          ₹{p.toLocaleString()}
                        </button>
            ...
            <div className="bmf" style={s.footer}>
              ...
              {negoStatus === "agreed"
                ...
                : negoStatus === "replied"
                  ? <button onClick={sendCounter} disabled={loading} style={{ ...s.priBtn, opacity: loading ? 0.7 : 1 }}>
                      {loading ? "Sending…" : "Send Counter →"}
                    </button>
                  : <button disabled style={{ ...s.priBtn, opacity: 0.4, cursor: "not-allowed" }}>
                      Waiting for artist…
                    </button>
              }
            </div>
          ...
          <div style={s.body}>
            <div style={s.card}>
              ...
              <div style={{ borderTop: "1px solid #e2e8f0", marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: "#1e3a8a" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div
                  ...
                  style={{ ...s.payOpt, borderColor: payMode === "advance" ? "#1e3a8a" : "#e2e8f0", background: payMode === "advance" ? "#eff6ff" : "#fff" }}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={s.payDesc}>Remaining ₹{balanceAmount.toLocaleString()} collected on event day.</div>
                      </div>
                    </div>
                    <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: "#1e3a8a", flexShrink: 0, marginLeft: 8 }}>
                ...
                <div
                  ...
                  style={{ ...s.payOpt, borderColor: payMode === "full" ? "#1e3a8a" : "#e2e8f0", background: payMode === "full" ? "#eff6ff" : "#fff" }}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={s.payDesc}>Full amount held securely. Released to {artist.name} after your event.</div>
                      </div>
                    </div>
                    <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: "#1e3a8a", flexShrink: 0, marginLeft: 8 }}>
                      ₹{agreedPrice?.toLocaleString()}
            ...
            <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "12px 14px" }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#64748b", marginBottom: 8, fontFamily: "'Nunito',sans-serif", textTransform: "uppercase", letterSpacing: 0.5 }}>
                Payment Breakdown
              </div>
              <div style={s.row}>
                <span style={{ color: "#64748b" }}>You pay now</span>
                <span style={{ fontWeight: 800, color: "#1e3a8a" }}>
                  ₹{(payMode === "advance" ? advanceAmount : agreedPrice)?.toLocaleString()}
                </span>
              </div>
              {payMode === "advance" && (
                <div style={s.row}>
...
const s = {
  ...
  stepDot:     { width: 24, height: 24, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, fontFamily: "'Nunito',sans-serif", flexShrink: 0 },
